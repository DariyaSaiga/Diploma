...
# ✅ Исправленный импорт (без Diploma.backend)
try:
    from bottleneck_fusion import BottleneckFusion  # если файл в корне
except ImportError:
    try:
        from backend.bottleneck_fusion import BottleneckFusion  # если файл в backend/
    except ImportError:
        print("⚠️  ОШИБКА: Не найден файл bottleneck_fusion.py")
        print("Проверьте структуру проекта.")
        raise

# ✅ Используем BottleneckDataset вместо MoseiDataset
...
def run_batch(model, batch: dict, model_type: str, return_domains: bool = False):
    """Forward pass. Если return_domains=True, возвращает (logits, domain_data, recon_data)."""
    if model_type == "text":
        return model(
...
# ──────────────────────────────────────────────────────────────────────────────
# Domain Separation Loss Functions
# ──────────────────────────────────────────────────────────────────────────────

def compute_separation_loss(domain_data: dict) -> torch.Tensor:
    """
    Separation loss: минимизируем косинусное сходство между invariant и private.
    Цель: разделить информацию между доменами.
    """
    losses = []
    ...
    for modality in ['text', 'audio', 'visual']:
        ...
        if inv is not None and priv is not None:
            # Cosine similarity между invariant и private
            cos_sim = torch.nn.functional.cosine_similarity(inv, priv, dim=1)
            # Минимизируем similarity (делаем их ортогональными)
            loss = cos_sim.mean()
            losses.append(loss)
...
def compute_invariant_loss(domain_data: dict, labels: torch.Tensor) -> torch.Tensor:
    """
    Invariant loss: одна и та же эмоция должна иметь похожий invariant сигнал.
    Используем triplet-like подход: pull same-emotion пары, push different-emotion.
    """
    margin = 1.0
    losses = []
    ...
    for modality in ['text', 'audio', 'visual']:
        ...
        # Вычисляем pairwise distances
        # inv: (B, hidden_dim)
        dists = torch.cdist(inv, inv)  # (B, B)

        # Для каждого сэмпла, найти расстояния до других с той же эмоцией
        same_emotion = (labels.unsqueeze(0) == labels.unsqueeze(1)).float()  # (B, B)
        same_emotion.fill_diagonal_(0)  # исключаем сам на себя

        # Среднее расстояние до same-emotion пар
        same_dists = (dists * same_emotion).sum(dim=1) / same_emotion.sum(dim=1).clamp(min=1)

        # Расстояния до different-emotion пар
        ...
        # Triplet-like loss: same должны быть ближе, чем different с margin
...
def compute_reconstruction_loss(recon_data: dict) -> torch.Tensor:
    """
    Reconstruction loss: восстанавливаем исходные features из [invariant || private].
    Цель: убедиться что информация сохранена в обоих доменах.
    """
    losses = []
...
def train_one_epoch(
    ...
    use_domain_separation: bool = False,  # Multi-loss training для bottleneck
    alpha_sep: float = 0.1,
    alpha_inv: float = 0.05,
    alpha_rec: float = 0.01,
) -> tuple:
    """
    Возвращает (loss, acc, f1, epoch_branch_losses).

    Если use_domain_separation=True и model_type="bottleneck":
        L_total = L_task + α_sep*L_sep + α_inv*L_inv + α_rec*L_rec
    """
    model.train()
    total_loss = 0.0
    ...
    for batch in loader:
        ...
        elif use_domain_separation and model_type == "bottleneck":
            # ── Multi-loss training для domain-separated bottleneck ────────
            result = run_batch(model, batch, model_type, return_domains=True)
            outputs, domain_data, recon_data = result

            # Вычисляем все losses
            L_task = criterion(outputs, labels)
...
def save_config(args, exp_dir: str) -> None:
    ...
    lines = [
        ...
        f"use_domain_sep       = {args.use_domain_sep}",
        f"alpha_sep            = {args.alpha_sep}",
        f"alpha_inv            = {args.alpha_inv}",
        f"alpha_rec            = {args.alpha_rec}",
...
def main() -> None:
    ...
    # ── Domain Separation Loss (для bottleneck модели) ──────────────────────
    parser.add_argument("--use_domain_sep", action="store_true",
                        help="Включить multi-loss training с domain separation")
    parser.add_argument("--alpha_sep", type=float, default=0.1,
                        help="Вес separation loss (L_sep)")
    parser.add_argument("--alpha_inv", type=float, default=0.05,
                        help="Вес invariant domain loss (L_inv)")
    parser.add_argument("--alpha_rec", type=float, default=0.01,
                        help="Вес reconstruction loss (L_rec)")

    # ── Загрузка претренированной модели ──────────────────────────────────
    parser.add_argument("--pretrained_path", type=str, default=None,
                        help="Путь к файлу с весами для загрузки (stage1/best_model.pt)")
    ...
    # ── Датасеты ──────────────────────────────────────────────────────────
    # ✅ Используем make_bottleneck_loaders для автоматической загрузки и collate_fn
    print(f"Загружаю датасет из {args.data_path}...")
    ...
    # ── Веса классов (загружаем вручную для WeightedRandomSampler) ────────
    ...
    # ⚠️  NOTE: Если нужен WeightedRandomSampler, нужно переписать логику
    # (текущая реализация не совместима с bottleneck_loaders)
    if args.use_sampler:
        print("  ⚠️  ВАЖНО: WeightedRandomSampler не поддерживается с bottleneck loaders")
        print("  Используем обычный shuffle=True для train_loader")

    # ── Модель ────────────────────────────────────────────────────────────
    model = build_model(args).to(device)

    # ── Загрузка претренированных весов ────────────────────────────────────
    if args.pretrained_path:
        if os.path.exists(args.pretrained_path):
            print(f"📥 Загружаю претренированные веса из: {args.pretrained_path}")
            model.load_state_dict(torch.load(args.pretrained_path, map_location=device))
            print(f"✅ Веса загружены!")
        else:
            print(f"⚠️  WARNING: Файл {args.pretrained_path} не найден, начинаем с нуля")
    ...
    if args.use_domain_sep and args.model == "bottleneck":
        print(f"🔥 Domain Separation ENABLED: α_sep={args.alpha_sep} | α_inv={args.alpha_inv} | α_rec={args.alpha_rec}")
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss, train_acc, train_f1, branch_losses = train_one_epoch(
            ...
            use_domain_separation=args.use_domain_sep and args.model == "bottleneck",
