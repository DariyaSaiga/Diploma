"""
Train — Baseline 2: BERT (frozen) + Conv1D + MulT-style Cross-Modal Attention
...
from baseline2_model import CrossModalFusionModel
...
SAVE_PATH = "/content/drive/MyDrive/Дипломка_правильная/results/baseline2_best.pt"
...
def main():
    ...
    print("Model: Baseline 2 — BERT(frozen) + Conv1D + MulT-style Cross-Modal Attention\n")
    ...
    print(f"\nФИНАЛЬНЫЕ РЕЗУЛЬТАТЫ — BASELINE 2 (MulT-style Cross-Modal Attention)")
    print(f"{'='*65}")
    ...
    print(f"\n  Сравнение (ориентир):")
    print(f"    Proposed (Bottleneck) : Avg.F1=0.4854")
    print(f"    Baseline 2 (этот)     : Avg.F1={test_f1:.4f}")
    print(f"    Baseline 1 (Late Fus) : Avg.F1=?  (запусти train_baseline1.py)")
