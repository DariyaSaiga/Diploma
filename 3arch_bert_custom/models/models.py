...
class BottleneckLayer(nn.Module):
    ...
    def forward(self, text, audio, vision, bottleneck):
        ...
        t, _ = self.self_attn_t(text, text, text)
...
class BottleneckFusionModel(nn.Module):
    ...
    def __init__(self):
        ...
        # ── Text encoder — Linear проекция precomputed BERT embeddings ────────────
        # Статья: DBA (He et al., 2024) — BERT-processed features как входные данные
        # Датасет mosei_bert_custom: text уже [N, 50, 768], BERT не нужен онлайн
    ...
    def forward(self, text, audio, vision, audio_mask, vision_mask):
        # text: [B, 50, 768] — precomputed BERT embeddings
        B = text.size(0)
        ...
        for layer in self.layers:
            text, audio, vision, bottleneck = layer(text, audio, vision, bottleneck)
...
if __name__ == "__main__":
    ...
    print(f"\nВход  — text  : {batch['text'].shape}")
    print(f"Вход  — audio : {batch['audio'].shape}")
