...
# CONFIG
# =========================
FILE_PATH = "mosei.hdf5"
SAVE_PATH = "mosei_preprocessed.pkl"
...
with h5py.File(FILE_PATH, "r") as f:
    ...
    for sid in all_ids:
        ...
        samples.append({
            "id": sid,
            "label": int(np.argmax(emotion_vals))
        })
...
train_ids, test_ids, train_labels, test_labels = train_test_split(
    ids, labels, test_size=0.15, stratify=labels, random_state=42
)

train_ids, val_ids, train_labels, val_labels = train_test_split(
    train_ids, train_labels, test_size=0.176, stratify=train_labels, random_state=42
)
...
with h5py.File(FILE_PATH, "r") as f:
    for s in samples:
        ...
        arr, mask = pad(arr, TARGET_LEN)

        audio[sid] = arr
        audio_mask[sid] = mask

audio_scaler = StandardScaler()
audio_scaler.fit(np.vstack([audio[i] for i in train_ids]))

for sid in audio:
    audio[sid] = audio_scaler.transform(audio[sid]).astype(np.float32)
...
with h5py.File(FILE_PATH, "r") as f:
    for s in samples:
        ...
        visual[sid] = arr
        visual_mask[sid] = mask

visual_scaler = StandardScaler()
visual_scaler.fit(np.vstack([visual[i] for i in train_ids]))

for sid in visual:
    visual[sid] = np.clip(
        visual_scaler.transform(visual[sid]),
        -5, 5
    ).astype(np.float32)

# =========================
# TEXT (BERT PRECOMPUTED)
# =========================
text = {}
...
with h5py.File(FILE_PATH, "r") as f:
    for s in samples:
        ...
        words = []
        for t in raw:
            w = t.decode("utf-8") if isinstance(t, bytes) else str(t)
            if w not in ["sp", "sil", "<unk>", ""]:
                words.append(w)

        sentence = " ".join(words)
        text[sid] = get_bert_embedding(sentence)

# =========================
# BUILD DATASET
# =========================
label_map = {s["id"]: s["label"] for s in samples}
...
def build(ids):
    ...
    for sid in ids:
        out.append({
            "id": sid,
            "audio": audio[sid],
            "audio_mask": audio_mask[sid],
            "visual": visual[sid],
            "visual_mask": visual_mask[sid],
            "text": text[sid],
            "label": label_map[sid]
        })
    return out
